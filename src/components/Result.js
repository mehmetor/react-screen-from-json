...
//import { updateConfigScreen } from "../config/configSlice";

const Result = () => {
	let dispatch = useDispatch();
	const { config } = useSelector((s) => s.slice);
	...
	function onCopy() {
		//dispatch(updateConfigScreen({ screen: Screen, name: state.name }));
		...
		<React.Fragment>
			...
			<div className="options">
				<TextArea
					...
					// }}
					height={500}
					value={JSON.stringify({ ScreenConfig: config }, null, 2)}
					style={{
